...
This document captures the architectural decisions made so far for `executions-v2`.

The goal is to keep the model simple while we build the readiness pipeline first, then evolve execution management later.

## Core Model

Executor v2 currently models execution state as three separate persistent surfaces:
...
Readiness lives in `executions-v2/readiness`.
...
We decided that readiness-specific SQL should live inside `executions-v2`, not inside the `tasks` module.
...
We decided not to query the agent repository directly from executor v2.

Instead:

- `executions-v2` imports `AgentsModule`
- readiness uses `AgentsService`

Reason:

- agent retrieval belongs to the agents domain
- executor v2 may depend on that domain, but should not reach into its repository directly
...
We decided that all v2 execution surfaces live under `/executions-v2`.

Current endpoints:

- `/executions-v2/queue`
- `/executions-v2/active`
- `/executions-v2/history`
