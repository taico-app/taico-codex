...
import type { ContextTagResponseDto } from 'shared';
...
export const TagInput: React.FC<TagInputProps> = ({
  ...
  const [suggestions, setSuggestions] = useState<ContextTagResponseDto[]>([]);
  const [showSuggestions, setShowSuggestions] = useState(false);
  const [selectedIndex, setSelectedIndex] = useState(-1);
  const [allTags, setAllTags] = useState<ContextTagResponseDto[]>([]);
    ...
    const searchTags = () => {
      if (inputValue.trim().length > 0) {
        ...
        const filtered = allTags
          .filter((tag: ContextTagResponseDto) =>
